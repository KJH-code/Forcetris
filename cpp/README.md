...
smooth instead of banding into rectangles. The second is fire: sixteen
frames of a flame, generated by warping four octaves of value noise
through a taper that is wide at the foot and pinched at the tip, and
then through a heat ramp from deep red to white. The noise lattice wraps
over exactly the distance the frames scroll, so the strip loops with
nothing to see at the seam, and each frame leans differently, so cycling
them licks rather than flickers.

That flame does three jobs. Overdrive sets the room alight - tongues
stand up off the floor and down both margins, banking as the ignition
runs out - and every one of them is tested against the play column and
dropped if it would touch it, so the stack, the ghost and the previews
are never hidden by the fire, on any layout. Every centred panel is a
plate off the same forge - dark iron, a bevel lit from above, rivets,
a rim that pulses like metal fresh out of the coals - standing in
flames that lick up its flanks and out from under its foot. And the
wordmark burns: the mark beside FORCETRIS is one tongue off the same
strip, cycling. The ladder still means what it says under the fuse:
attack and pace climb it monotonically, D through X.
